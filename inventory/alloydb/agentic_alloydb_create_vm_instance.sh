...
# Purpose: 
#   - Ensure a GCE VM (used as an AlloyDB "virtual instance" host) exists/runs
#   - Copy DDL-related scripts to the VM
#   - SSH into the VM and execute the DDL creation script for AlloyDB
#
# Requirements:
#   - gcloud SDK authenticated (ACCOUNT must have required IAM permissions)
#   - agentic_config.param present and correctly defines required variables:
#       ACCOUNT, INSTANCE_NAME, ZONE_NAME, MACHINE_NAME, IMAGE_FAMILY, 
#       IMAGE_PROJECT, TAG, SCOPES, USERNAME


. "$ALLOYDB_INVENTORY_CONFIG"
