...
CREATE TABLE IF NOT EXISTS alloydb_demo.transactions (
  transaction_id  BIGSERIAL PRIMARY KEY,
  order_id        TEXT,
  order_ts        TIMESTAMPTZ NOT NULL,
  store_id        INT REFERENCES alloydb_demo.stores(store_id),
  channel         TEXT,
  sku             TEXT REFERENCES alloydb_demo.products(sku),
  quantity        INT NOT NULL,
  unit_price      NUMERIC(12,2) NOT NULL,
  country         TEXT
);

CREATE MATERIALIZED VIEW IF NOT EXISTS alloydb_demo.mv_daily_demand AS
SELECT store_id, sku, date_trunc('day', order_ts)::date AS day, SUM(quantity) AS qty
FROM alloydb_demo.transactions
GROUP BY 1,2,3;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE t.typname = 'po_status' AND n.nspname = 'alloydb_demo'
  ) THEN
    EXECUTE 'CREATE TYPE alloydb_demo.po_status AS ENUM
      (''draft'',''pending_approval'',''approved'',''sent'',''received'',''closed'',''cancelled'')';
  END IF;
END$$;

CREATE TABLE IF NOT EXISTS alloydb_demo.docs (
  doc_id      BIGSERIAL PRIMARY KEY,
  sku         TEXT NULL REFERENCES alloydb_demo.products(sku),
  doc_type    TEXT,
  source_url  TEXT,
  body        TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS alloydb_demo.purchase_orders (
  po_id        BIGSERIAL PRIMARY KEY,
  supplier_id  INT NOT NULL REFERENCES alloydb_demo.suppliers(supplier_id),
  status       alloydb_demo.po_status NOT NULL DEFAULT 'draft',
  created_at   TIMESTAMPTZ DEFAULT now(),
  created_by   TEXT,
  approved_by  TEXT,
  expected_at  DATE,
...
CREATE TABLE IF NOT EXISTS alloydb_demo.purchase_order_lines (
  po_line_id   BIGSERIAL PRIMARY KEY,
  po_id        BIGINT NOT NULL REFERENCES alloydb_demo.purchase_orders(po_id) ON DELETE CASCADE,
  sku          TEXT   NOT NULL REFERENCES alloydb_demo.products(sku),
  qty          INT    NOT NULL,
